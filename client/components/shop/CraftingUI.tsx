...
export default function CraftingUI() {
  ...
  const craft = (recipeId: string) => {
    ...
    for (const k of Object.keys(r.materials)) {
      if ((materials[k] || 0) < (r.materials as any)[k]) {
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="font-medium">
                  {Catalog.find((c) => c.id === r.id)?.name || r.id}
                </div>
              ...
              <div className="text-right">
                <button
                  onClick={() => craft(r.id)}
                  className="mt-2 inline-flex items-center gap-2 rounded-md bg-emerald-600 text-white px-3 py-1 text-sm font-semibold"
                >
                  Craft
                </button>
              </div>
